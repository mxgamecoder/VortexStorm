...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "VortexStorm")
       await eco.deduct(users, "VortexStorm", balance.wallet);
  ...
  try{
   ...
   switch (value) {
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"VortexStorm", 1000);
         const add2 = eco.giveCapacity(user, "VortexStorm", 100000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "VortexStorm", 10000);
          const add3 = eco.giveCapacity(user, "VortexStorm", 10000000);
