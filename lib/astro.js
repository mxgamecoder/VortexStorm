...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "VortexStorm"],
    ...
    getMessage: async (message) => {
      let defaultMessage = { conversation: "Hello 🌍" };
      if (store) {
      ...
      for (mek of callData.messages) {
        ...
        if (rawText && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          isCommand =
            ...
            (messageData.isAsta &&
              /2349021506036|923004591719|17863688449/g.test(botNumber) &&
              messageBody[0] == ",");
              ...
              if (userData.ban === "true") {
                ...
                messageData.reply(
                  "Hey " +
                    ...
                    ",\n_You are banned from using commands motherfucker 😂._"
                );
              }
            } catch (error) {
        ...
        events.commands.map(async (command) => {
          if (typeof command.on === "string") {
            ...
            if (commandName === "main" && shouldExecute) {
              ...
              messageData.text &&
              commandName === "text" &&
              /text|txt|true|smd|storm/gi.test(command.quoted) &&
              messageData.quoted &&
    ...
    async (groupParticipantsUpdate) => {
      try {
        ...
        events.commands.map(async (command) => {
          ...
          if (/on|true|main|all|storm|smd/gi.test(command.group)) {
            try {
              command.function(groupData, { store: store, Void: AstaConn });
            } catch (error) {
  ...
  AstaConn.ev.on("groups.upsert", async (groupsUpsert) => {
    try {
      events.commands.map(async (command) => {
        if (
          /on|true|main|all|storm|smd/gi.test(
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      if (
        ...
        log(
          "VortexStorm RESETTING SESSION_ID" +
            (AstaConn.authState.creds?.myAppStateKeyId
              ? ""
              : " STORM PLEASE RESTART PROCESS ONCE CONNECTED") +
            "!"
        );
      ...
      try {
        ...
        if (_0x506b6e && _0x506b6e.plugins) {
          log("VortexStorm Plugins Installed ✅");
          _0xd559f7 = {
            ..._0x506b6e.plugins,
            ..._0xd559f7,
          };
        ...
        if (Object.keys(_0xd559f7 || {}).length > 0) {
          ...
          log("\n✅ VortexStorm External Plugins Installed!");
        }
      } catch (_0x42a37a) {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "VortexStorm") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "VortexStorm") => {
    ...
    try {
      ...
      if (
        (((_0x171bcf && _0x3bceba && global.style > 0) || !_0x3bceba) &&
          /text|txt|nothing|smd|storm/.test(global.userImages)) ||
        _0x2e5e1f == "text"
      ) {
        _0x237b02 = {
...
<head>
  ...
  <title>VortexStorm Bouncing Text</title>
  <style>
    body, html {
      margin: 0;
      padding: 0;
...
<body>
  <div class="container">
    <h1 class="bounce">VortexStorm</h1>
  </div>
</body>
</html>
`;
...
app.listen(randomPort, () =>
  console.log(
    "VortexStorm Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  let decodedSessionId = ("" + sessionId)
    ...
    .replace(/^STORM_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "")
    .replace(/Secktor;;;/gi, "")
    .replace(/Astro;;;/gi, "")
    .replace(/Asta;;;/gi, "")
    .trim();
  ...
  const officialRepoPath = "/mxgamecoder/";
  ...
  if (isOfficialRepo) {
    ...
    if (
      ...
      try {
        ...
        log(
          "EMPTY SESSION_ID FROM VortexStorm SERVER\nPLEASE SCAN THE QR AGAIN FROM [ " +
            global.scan +
            " ]\n\n\nERROR: ",
          error
        );
