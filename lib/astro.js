...
async function syncdb() {
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "STORM") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "STORM") => {
...
app.listen(randomPort, () =>
  console.log(
    "STORM Server listening on http://localhost:" + randomPort + "/ "
  )
);
